...
<!--            <sch:assert test="not($specification_inspire) or ($specification_date and $specification_dateType)">
            ...
            <sch:report test="$has_specification_title">
                <sch:value-of select="$loc/strings/report.M44.spec/div"/><sch:value-of select="$has_specification_title"/>, 
                [[[<sch:value-of select="$specification_title"/>]]] 
                [[[<sch:value-of select="$lang"/>]]] ,
                IT:<sch:value-of select="$isItMetadata"/> 
                EN:<sch:value-of select="$isEnMetadata"/> 

                (<sch:value-of select="$specification_date"/>, <sch:value-of select="$specification_dateType"/>)
            </sch:report>
