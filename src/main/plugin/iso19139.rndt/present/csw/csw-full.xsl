...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:csw="http://www.opengis.net/cat/csw/2.0.2"
   ...
   <xsl:template match="gmd:MD_Metadata|*[@gco:isoType='gmd:MD_Metadata']">
      ...
      <csw:Record>
         ...
         <xsl:for-each select="$identification/gmd:citation/gmd:CI_Citation/gmd:title">
            <dc:title>
         ...
         <xsl:for-each
            select="$identification/gmd:descriptiveKeywords/gmd:MD_Keywords/gmd:keyword[not(@gco:nilReason)]">
            <dc:subject>
               <xsl:apply-templates mode="localised" select=".">
                  <xsl:with-param name="langId" select="$langId"/>
               </xsl:apply-templates>
         ...
         <xsl:for-each select="$identification/gmd:topicCategory/gmd:MD_TopicCategoryCode">
            <dc:subject>
               <xsl:value-of select="."/>
        ...
        gmd:resourceConstraints/*[@gco:isoType='gmd:MD_LegalConstraints']">
         ...
         <xsl:for-each select="gmd:parentIdentifier/gco:CharacterString[text()]">
            <dc:relation>
               <xsl:value-of select="."/>
            </dc:relation>
         </xsl:for-each>
                  ...
                  <dc:URI>
                     <xsl:if test="gmd:protocol">
                        <xsl:attribute name="protocol">
                           <xsl:variable name="href" select="gmd:protocol/gmx:Anchor/@xlink:href"/>
                           <xsl:value-of select="if($href) then $href else gmd:protocol/*/text()"/>
                        </xsl:attribute>
                     </xsl:if>

                     <xsl:if test="gmd:applicationProfile">
                        <xsl:attribute name="profile">
                           <xsl:variable name="href" select="gmd:applicationProfile/gmx:Anchor/@xlink:href"/>
                           <xsl:value-of select="if($href) then $href else gmd:applicationProfile/*/text()"/>
                        </xsl:attribute>
                     </xsl:if>
